...
class Message(BaseModel):
    role: MessageRole
    content: str
    images: Optional[List[str]] = None  # Base64 string atau URL gambar untuk Multimodal Vision
    tool_calls: Optional[List[ToolCall]] = None
    tool_call_id: Optional[str] = None
    name: Optional[str] = None
    timestamp: datetime = Field(default_factory=datetime.now)

    def to_llm_dict(self, backend: str = "gemini") -> Dict[str, Any]:
        """Format pesan ke struktur API (mendukung input gambar/multimodal vision)."""
        payload = {"role": self.role.value, "content": self.content}
        
        # Dukungan multimodal vision standar
